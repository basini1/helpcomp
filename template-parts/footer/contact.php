<div class="flex flex-col gap-y-6">
    <address class="not-italic max-w-60">
        <?php echo get_field('company_full-name', 'option') ? '<strong class="block mb-6 font-medium text-white">' . get_field('company_full-name', 'option') . '</strong>' : null; ?>
        ...
        <p class="flex flex-col items-start gap-y-1">
            <?php while (have_rows('company_phones', 'option')) : the_row(); ?>
                <a class="flex flex-wrap gap-x-3 group" href="tel:<?php echo str_replace(' ', '', get_sub_field('company_phone', 'option')); ?>">
                    ...
                    <span class="block transition-colors duration-300 text-helpcompGray group-hover:text-helpcompBlue"><?php echo get_sub_field('company_phone', 'option'); ?></span>
                </a>
            <?php endwhile; ?>
        </p>
    <?php endif; ?>
